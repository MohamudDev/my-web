...
import { Trophy, Sun, Moon, RotateCcw, History } from "lucide-react";
...
export default function ScoreBoardPage() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 flex flex-col items-center justify-between pb-8 pt-6 px-4 md:px-8 font-sans ${
      ...
      <header className="w-full max-w-2xl flex items-center justify-between z-10 mb-6">
        <h1 className="text-xl md:text-2xl font-black tracking-widest text-primary uppercase">Score Board</h1>
        <div className="flex items-center gap-3">
          <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2.5 rounded-full transition-all duration-300 ${isDarkMode ? "bg-white/5 hover:bg-white/10 text-primary" : "bg-black/5 hover:bg-black/10 text-primary"}`} aria-label="Toggle Theme">
            {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
          </button>
          <button onClick={handleRestart} className={`p-2.5 rounded-full transition-all duration-300 ${isDarkMode ? "bg-white/5 hover:bg-white/10 text-text-secondary hover:text-white" : "bg-black/5 hover:bg-black/10 text-gray-500 hover:text-black"}`} aria-label="Reset Match">
            <RotateCcw size={20} />
          </button>
        </div>
      </header>
      ...
      <AnimatePresence>
        {isGameOver && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className={`w-full max-w-sm p-8 rounded-[32px] border text-center relative overflow-hidden ${isDarkMode ? "bg-[#0A1A1F] border-white/10" : "bg-white border-gray-200 shadow-2xl"}`}>
              <div className="absolute top-0 left-0 w-full h-2 bg-primary"></div>
              <motion.div animate={{ rotate: [0, 10, -10, 10, 0], scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 2.5 }} className="inline-block p-5 bg-primary/20 rounded-full text-primary mb-6">
                <Trophy size={56} />
              </motion.div>
              <h2 className={`text-3xl font-black tracking-wide uppercase mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}>🏆 {winner} Winner</h2>
              <p className="text-primary font-bold text-sm uppercase tracking-widest mb-6">Score of 101 Reached!</p>
              <button onClick={handleRestart} className="w-full py-4 bg-primary hover:bg-primary-hover text-bg-main font-black rounded-xl transition-all shadow-[0_0_20px_rgba(234,180,100,0.4)] flex items-center justify-center gap-2">
                <RotateCcw size={18} />
...
function TeamCard({ teamName, score, input, setInput, preview, history, isGameOver, isWinner, isLoser, onKeyDown, isDarkMode }) {
  return (
    <motion.div className={`p-6 rounded-[24px] border transition-all duration-300 flex flex-col justify-between ${
      ...
      <div className="text-center">
        <h2 className="text-xl font-bold uppercase tracking-wider mb-2">{teamName}</h2>
        <div className="my-4">
          <span className="text-6xl md:text-7xl font-black tracking-tight text-primary block">{score}</span>
          <span className="text-xs text-text-secondary block mt-1">/ 101</span>
        </div>
      </div>

      <div className="mt-4 flex flex-col gap-3">
        <div className={`flex items-center p-3 rounded-xl border transition-all ${
      ...
      <div className={`mt-6 border-t pt-4 ${isDarkMode ? "border-white/5" : "border-gray-100"}`}>
        ...
        <div className={`rounded-xl p-3 h-32 overflow-y-auto flex flex-col gap-1.5 ${
          ...
          {history.length === 0 ? (
            ...
            history.map((item, idx) => (
              <div key={idx} className={`flex justify-between items-center text-xs font-medium pb-1 border-b ${isDarkMode ? "border-white/5" : "border-gray-200"}`}>
